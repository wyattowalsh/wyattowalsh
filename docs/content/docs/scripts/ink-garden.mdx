...
`scripts/art/ink_garden.py` — ~2027 lines. Shared utilities in `scripts/art/shared/` (import as `scripts.art.shared`).
...
# Generate the Ink Garden timelapse only
...
That command writes `living-inkgarden.gif`. It refreshes
`living-art-manifest.json` and `living-art-preview.html` only when the output
directory contains a valid, complete six-style canonical fleet. Otherwise it
reports the missing styles and leaves any existing index surfaces untouched.
...
- [Generative Art](/docs/scripts/generative-art) — Lorenz/neural/flow-field art (same `scripts.art.shared` utilities)
- [Living Art Modes](/docs/scripts/living-art-modes) — Timelapse generation, complete-fleet indexing, optimization, and publication contracts
- [Topography](/docs/scripts/topography) — Topographic contour art (`art/topography.py`)
