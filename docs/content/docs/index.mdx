...
| **Auth Helpers** (`scripts/spotify_auth.py`) | One-time Spotify refresh-token bootstrap for the supplemental music card |
...
| **Genetic Landscape** (`scripts/art/genetic_landscape.py`) | Evolutionary fitness terrain where repositories become adaptive peaks |
| **Physarum** (`scripts/art/physarum.py`) | Slime-mold transport network that links repositories as nutrient nodes |
| **Lenia** (`scripts/art/lenia.py`) | Continuous cellular automata seeded from repository history |
| **Ferrofluid** (`scripts/art/ferrofluid.py`) | Magnetic spike sculpture shaped by repository fields and portfolio energy |
...
| **README SVG** (`scripts/readme_svg.py`) | Card/block SVG helpers with embedded `@media` dark tokens (single file per card) |
