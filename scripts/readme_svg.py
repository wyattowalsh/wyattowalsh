...
class SvgThemeEffects:
    ...
    # --- tricked-out visual FX tokens ---
    card_glow_blur: int = 18
    card_glow_opacity: str = "0.14"
    sparkline_dash_total: int = 800
    sparkline_draw_duration: str = "2.4s"
    icon_pulse_duration: str = "3s"
    badge_shimmer_duration: str = "3.6s"
    card_fadein_duration: str = "0.6s"
    card_fadein_stagger: str = "0.12s"
    title_shadow_offset: int = 1
    title_shadow_opacity: str = "0.32"
    corner_dot_radius: float = 2.5
    corner_dot_opacity: str = "0.40"
    glass_inner_opacity: str = "0.06"
    glass_highlight_opacity: str = "0.04"
...
class SvgBlockRenderer:
    ...
    def render(self, block: SvgBlock) -> str:
        ...
        svg_lines = [
            ...
            # --- tricked-out animation keyframes ---
            "@keyframes card-fadein {",
            "  from { opacity: 0; transform: translateY(8px); }",
            "  to { opacity: 1; transform: translateY(0); }",
            "}",
            (
                ".card { opacity: 0; animation: card-fadein "
                f"{theme.effects.card_fadein_duration} ease-out forwards; }}"
            ),
            *[
                (
                    f'.card:nth-child({i + 1}) {{ animation-delay: '
                    f"calc({theme.effects.card_fadein_stagger} * {i}); }}"
                )
                for i in range(len(cards))
            ],
            "@keyframes sparkline-draw {",
            f"  from {{ stroke-dashoffset: {theme.effects.sparkline_dash_total}; }}",
            "  to { stroke-dashoffset: 0; }",
            "}",
            (
                ".sparkline { stroke-dasharray: "
                f"{theme.effects.sparkline_dash_total}; "
                f"stroke-dashoffset: {theme.effects.sparkline_dash_total}; "
                f"animation: sparkline-draw {theme.effects.sparkline_draw_duration} "
                "cubic-bezier(0.4, 0, 0.2, 1) forwards; }"
            ),
            "@keyframes icon-ring-pulse {",
            "  0%, 100% { stroke-opacity: 0.16; stroke-width: 1; }",
            "  50% { stroke-opacity: 0.38; stroke-width: 1.5; }",
            "}",
            (
                ".icon-ring-animated { animation: icon-ring-pulse "
                f"{theme.effects.icon_pulse_duration} ease-in-out infinite; }}"
            ),
            "@keyframes badge-shimmer {",
            "  0% { fill-opacity: 0.36; }",
            "  50% { fill-opacity: 0.56; }",
            "  100% { fill-opacity: 0.36; }",
            "}",
            (
                ".card-badge-pill { animation: badge-shimmer "
                f"{theme.effects.badge_shimmer_duration} ease-in-out infinite; }}"
            ),
            ".card-title-shadow { opacity: "
            f"{theme.effects.title_shadow_opacity}; "
            "filter: blur(1px); }",
            "</style>",
            (
            ...
            # accent glow filter for card depth
            (
                '<filter id="accentGlow" x="-20%" y="-20%" width="140%" height="140%">'
                f'<feGaussianBlur in="SourceGraphic" stdDeviation="{theme.effects.card_glow_blur}" />'
                "</filter>"
            ),
            # glass-morphism highlight gradient
            (
                '<linearGradient id="glassHighlight" x1="0%" y1="0%" x2="0%" y2="100%">'
                '<stop offset="0%" stop-color="#FFFFFF" stop-opacity="'
                f'{theme.effects.glass_highlight_opacity}"/>'
                '<stop offset="40%" stop-color="#FFFFFF" stop-opacity="0"/>'
                '<stop offset="100%" stop-color="#000000" stop-opacity="'
                f'{theme.effects.glass_inner_opacity}"/>'
                "</linearGradient>"
            ),
            "</defs>",
        ]
    ...
    def _render_card(
        ...
        # accent glow layer behind card for depth
        lines.append(
            '<rect width="'
            f"{width}"
            '" height="'
            ...
            f"{accent}"
            '" fill-opacity="'
            f"{theme.effects.card_glow_opacity}"
        ...
        # glass-morphism highlight overlay
        lines.append(
            '<rect width="'
            f"{width}"
            '" height="'
        ...
        if show_icon_lockup and (icon_data_uri or monogram):
            ...
            if icon_data_uri:
                ...
                lines.append(
                    '<circle class="icon-ring-animated" cx="36" cy="36" r="'
                    f"{theme.radii.icon_inner}"
                    '" fill="none" '
                    'stroke="'
                    f"{theme.palette.icon_ring_stroke}"
        ...
        shadow_dx = theme.effects.title_shadow_offset
        if family_policy.text.wrap_title_with_tspan:
            ...
            lines.append(
                (
                    '<text class="card-title card-title-shadow" x="'
                    f"{title_x + shadow_dx}"
                    '" y="'
                    f"{title_y + shadow_dx}"
                    '">'
        ...
        # corner accent dots
        dot_r = theme.effects.corner_dot_radius
        dot_op = theme.effects.corner_dot_opacity
        dot_inset = 8
        for cx, cy in [
            (dot_inset, dot_inset),
            (width - dot_inset, dot_inset),
            (dot_inset, self.card_height - dot_inset),
            (width - dot_inset, self.card_height - dot_inset),
        ]:
            lines.append(
                f'<circle cx="{cx}" cy="{cy}" r="{dot_r}" '
                f'fill="{accent}" fill-opacity="{dot_op}" />'
            )
    ...
    def _sparkline_path(
        ...
        """Generate a smooth cubic-bezier sparkline path for premium feel."""
        total = len(points) - 1
        if total <= 0:
            return ""
        min_val = min(points)
