...
| `METRICS_TOKEN` | Canonical repo: yes / forks: recommended | Classic GitHub PAT for richer GitHub reads (`read:user`, `repo`, `read:org`; add `gist` if you want gist-aware surfaces later). The historical `repositories` block still needs the probe lane to clear before it should return to production. |
...
| `SPOTIFY_REFRESH_TOKEN` | Canonical repo: optional / forks: optional | Spotify OAuth refresh token for the repo-owned `metrics-music.svg` supplemental card |
| `X_BEARER_TOKEN` | Canonical repo: optional / forks: optional | X bearer token for the repo-owned `metrics-posts.svg` supplemental card |
