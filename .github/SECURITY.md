...
Scheduled `generate-profile-metrics` already keeps `plugin_music: no` on
`lowlighter/metrics`. Spotify belongs on the supplemental-metrics step only.

#### Current workflow note (remediation in progress)

As of this writing, the manual `probe-full-metrics` lane in
`.github/workflows/profile-updater.yml` still uses `felipecrs/metrics` and may
still pass Spotify credentials via `plugin_music_token` in `with:`. That path
**violates** this policy. Remediation is in progress (Wave M/D will remove the
probe Spotify / `felipecrs` secret wiring). Until that lands, treat the probe
lane as non-compliant legacy behavior — do not copy that pattern into new jobs
or into the scheduled `lowlighter/metrics` steps.
