...
Fetches live metrics + history via `scripts/fetch_metrics.py` and `scripts/fetch_history.py`, then runs `uv run python -m scripts.cli generate living-art` to refresh the public six-style growth surface. The job stages the generated growth SVGs/GIFs plus `living-art-manifest.json` and `living-art-preview.html`, uploads the full staging bundle, and auto-commits the committed living-art assets.
