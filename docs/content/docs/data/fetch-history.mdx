...
The commit timestamps are used to compute daily activity density and day-by-day sampling. After a successful render, the same output directory is re-indexed into `living-art-manifest.json` and `living-art-preview.html`.
