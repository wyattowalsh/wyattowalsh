...
│   │   ├── auth.py             # One-time auth helpers (Spotify refresh token)
...
│   ├── readme_svg.py           # SVG card helpers (@media light/dark tokens)
...
│   ├── spotify_auth.py         # Spotify loopback auth-code helper for refresh tokens
│   └── art/
...
ink_garden.py / topography.py
  ├── art/shared/ (seed_hash, Noise2D, WorldState)
  └── numpy
...
| `readme_sections.py` | ~2495 | One coherent assembler |
| `readme_svg.py` | ~1740 | SVG card helpers (`@media` light/dark tokens) |
