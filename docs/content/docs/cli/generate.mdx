...
It also renders the Metrics section from whichever validated first-party
supplemental assets are present:

- `metrics-languages.svg` (supplemental)
...
Production no longer embeds `lowlighter/metrics` cards (`metrics.svg`,
`metrics.additional.svg`, `metrics.extra.svg`).
