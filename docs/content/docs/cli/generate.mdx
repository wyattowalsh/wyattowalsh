...
CLI options take precedence over config values. The command calls
`generate_banner(cfg=BannerConfig(...))` twice — once for light and once for
dark — after removing each exact prior target. Both variants must materialize
as fresh, non-empty, valid SVGs; either failure exits nonzero and cannot be
masked by a stale checked-out banner.
...
The background is optional. Without one, the generator writes a standard QR
PNG. The command removes the exact prior target and succeeds only when the
renderer returns the requested path containing a fresh, valid PNG; no-op,
partial, invalid, or redirected output fails nonzero and is cleaned up.
...
| `--output-dir` | Output directory; partial `--only` runs do not refresh index surfaces |
...
| `--only` | Restrict to one style: `inkgarden`, `topo`, `genetic`, `physarum`, `lenia`, or `ferrofluid` |
| `--output-dir` | Output directory; use a fresh directory for isolated style jobs |
| `--workers` | Parallel render workers |
...
It also renders the additive Metrics section from the lowlighter stack plus
whichever validated supplemental assets are present:

- `metrics.svg` (primary lowlighter)
- `metrics.additional.svg` (additional lowlighter)
- `metrics.extra.svg` (overflow lowlighter: reactions / followup)
- `metrics-habits.svg` (supplemental)
- `metrics-activity.svg` (supplemental)
- `metrics-music.svg` (supplemental; Spotify)
- `metrics-posts.svg` (supplemental; X)

Optional supplemental cards are omitted automatically when their assets are missing or invalid.
Lowlighter habits/activity/music stay off — those surfaces are supplemental-owned only.
