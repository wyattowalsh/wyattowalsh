...
Usage:
  uv run python -m scripts.art.animate [options]
  uv run python -m scripts.art.animate [options]
"""
...
def _build_stacked_svg(
    ...
    for i in range(1, n):
        ...
        css_lines.append(
            f".f{i}{{animation:{kf_name} {act_dur:.1f}s {delays[i]:.1f}s {easing} both}}"  # noqa: E501
        )
...
def main() -> None:
    ...
    if only and only in all_generators:
        ...
        logger.error(
            "Unknown style '{}'. Available: {}", only, list(all_generators.keys())
        )
        sys.exit(1)
    else:
    ...
    if opts.get("svg"):
        ...
        for key, (slug, gen_fn) in generators.items():
            ...
            animated_svg = _build_stacked_svg(
                frame_svgs, delays, total_duration=total_dur
            )
            out_path = out_dir / f"{slug}-animated.svg"
            out_path.write_text(animated_svg, encoding="utf-8")
            size_kb = out_path.stat().st_size // 1024
            logger.info(
                "{}: {} KB ({} frames) -> {}", slug, size_kb, svg_frames, out_path
            )
    ...
    logger.info(
        "Profile: {}  target_mat={:.3f}  frames={}", profile, target_mat, n_frames
    )
