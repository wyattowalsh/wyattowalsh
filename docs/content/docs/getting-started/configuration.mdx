...
| Field | Type | Default | Notes |
|---|---|---|---|
| `output_filename` | `str` | `"qr.png"` | Bare lowercase `.png` filename; no path components |
...
`WordCloudSettingsModel` in `config.py` is the top-level YAML config. `WordCloudSettings` in `scripts/word_clouds/generate.py` is the internal runtime generator config with `extra="forbid"`. Don't confuse them — runtime-only palette tokenization, style variants, and palette overrides are not persisted YAML fields.
</Callout>
