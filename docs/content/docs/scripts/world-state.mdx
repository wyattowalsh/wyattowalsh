---
title: World State
description: Shared environmental semantics that keep the living-art generators in sync.
---
...
The living-art suite uses one shared environmental model so the generators feel like different lenses on the same portfolio state instead of unrelated visualizations. A profile with late-night commit bursts, rising stars, and active merged PRs should read as the same moment whether you look at the garden or the map.
...
The same `WorldState` drives every living-art generator, but each renderer expresses it differently. A stormy night may become foggy ridges in Topography or denser canopies in Ink Garden.
</Callout>
