...
`WorldState` lives in `scripts/art/shared/world_state.py` (re-exported from `scripts.art.shared`) and is computed by `compute_world_state(metrics)`.
...
`scripts.art.shared` (implemented in `shared/color.py`) also exposes `wcag_contrast_ratio()` and `ensure_contrast()`. Those helpers exist because richer low-light scenes are only useful if labels and highlights remain readable against dark backgrounds.
